...
# Headcount Ratio Urban: índice de recuento de la pobreza (% de la población que figura como pobre) dentro de las áreas urbanas del país
# Intensity of Deprivation Urban: distancia promedio por debajo de la línea de pobreza de los que figuran como pobres en áreas urbanas.
...
# Indico la agregación en los nombres
names(country.by.WR) = paste(names(country.by.WR), "_agg", sep="")
...
heatmap.2(ds.cor,
          ...
          notecol="black",     
          trace="none",        
          margins = c(9,22), # 1er numero: margen inferior, 2do: margen izquierdo
...
# Boxplot
boxplot(scale(MPI_int[,3:8]), cex.axis = 0.8, main = "Boxplot de los atributos numéricos")
...
# Plot de atributo 
plot(MPI_int$MPI.Urban, main = "Plot de Intensity of deprivation rural", xlab="índice del valor", ylab="valor")
plot(sort(MPI_int$Intensity.of.Deprivation.Urban), main = "Plot de Intensity of deprivation rural", xlab="índice del valor", ylab="valor")
...
# bin_eq_width <- discretize(atributo2,"equalwidth", BINS_EQ.W)
# barplot(table(bin_eq_width), main = "Frecuencia de bins (eqWidth)")
...
# grafico el atributo original ordenado de menor a mayor
